...
function updateDashboardStats() {
  ...
  const activeUsers = allUsers.filter(user => user.status === "active" && !user.deleted).length;
      ...
      row.innerHTML = `
        ...
        <td>
          <span class="badge ${
            user.status === "banned"
              ? "badge-banned"
              : (user.status === "deleted" || user.deleted) ? "badge-deleted" : "badge-active"
          }">
            ${
              user.status === "banned"
                ? "Banned"
                : (user.status === "deleted" || user.deleted) ? "Deleted" : "Active"
            }
          </span>
        </td>
...
function attachUserActions() {
  ...
  document.querySelectorAll(".delete-btn").forEach((btn) => {
    ...
    btn.addEventListener("click", async () => {
      ...
      try {
        // Soft-delete via admin panel: mark user as deleted
        await updateDoc(doc(db, "users", userId), {
          status: "deleted",
          deleted: true,
        ...
        showToast("User marked deleted (soft)");
