...
const TRANSLATIONS = {
  ...
  fr: {
    ...
    checkBack:     "Revenez plus tard pour voir plus de correspondances.",
    refresh:       "Actualiser",
...
let matchId = null;
if (window.location.pathname.endsWith('chat.html') || window.location.pathname.endsWith('match.html')) {
  matchId = urlParams.get("matchId");
  if (!matchId) {
    const storedPartner = JSON.parse(localStorage.getItem('amorwa_chatPartner') || 'null');
    matchId = storedPartner?.matchId || null;
  }
  if (!matchId) {
    console.error("No matchId found in URL or localStorage");
  }
}
...
window.sendMessage = async function (event) {
  ...
  if (!matchId) {
    console.warn('[script.js] No matchId available for sending message');
    window.showToast && window.showToast("Chat is not initialized correctly.") || alert("Chat is not initialized correctly.");
    return;
  }

  try {
    console.debug('[script.js] Sending message to match:', matchId);
    await addDoc(collection(db, "matches", matchId, "messages"), {
      text,
      sender: user.uid,
      createdAt: serverTimestamp()
    });
    console.debug('[script.js] Message sent successfully');
    input.value = "";
    input.focus();
  } catch (err) {
    console.error("[script.js] sendMessage failed", err.code, err.message);
...
function initChatPage() {
  ...
  console.debug('[script.js] initChatPage() called, matchId:', matchId);
  if (!chatMessages) {
    console.warn('[script.js] Chat messages container not found');
    return;
  }

  if (!matchId) {
    console.error('[script.js] No matchId available for chat initialization');
    return;
  }
  ...
  onAuthStateChanged(auth, async (user) => {
    ...
    console.debug('[script.js] User authenticated, loading chat for match:', matchId);
    ...
    try {
      console.debug('[script.js] Fetching match document:', matchId);
      const matchDoc = await getDoc(doc(db, "matches", matchId));
      if (matchDoc.exists()) {
        const matchData = matchDoc.data();
        console.debug('[script.js] Match data:', matchData);
        const otherUserId = Array.isArray(matchData.users)
          ? matchData.users.find((uid) => uid !== user.uid) || matchData.users[0]
          : null;

        if (otherUserId) {
          console.debug('[script.js] Loading other user profile:', otherUserId);
        ...
        console.warn('[script.js] Match document does not exist:', matchId);
      }
    } catch (err) {
      console.error('[script.js] Error loading match/user:', err);
    }

    console.debug('[script.js] Setting up message listener for match:', matchId);
    const q = query(
    ...
    unsubscribeChat = onSnapshot(q, (snapshot) => {
      console.debug('[script.js] Messages snapshot received:', snapshot.size);
      ...
      console.error('[script.js] Message listener error:', err);
    });
  });
}
...
window.deleteAccount = async function () {
  const confirmDelete = confirm(
    "Are you sure you want to delete your account? This will mark your profile as deleted and hide it from others."
  );
  ...
  try {
    // Soft-delete: mark the Firestore user doc as deleted and inactive
    ...
    // Optionally attempt to delete the Auth user (may require recent login). Ignore failures.
    try {
      await deleteUser(user);
    } catch (e) {
      console.warn('Could not delete auth user (requires recent login). Auth account may remain but profile is soft-deleted.', e);
    }

    (window.showToast && window.showToast("Account marked deleted.")) || alert("Account marked deleted.");
