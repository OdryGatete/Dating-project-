...
async function handleLike(targetUserId) {
  ...
  const likeRef = doc(db, "likes", `${currentUser.uid}_${targetUserId}`);
  ...
  try {
    ...
    showToast(typeof window.t === 'function' ? t('matchFound') : 'Unable to like user. Please try again.');
  }
}
...
async function createMatch(user1, user2) {
  ...
  await setDoc(matchRef, {
    ...
    createdAt: existingMatch.exists() ? existingMatch.data().createdAt || Date.now() : Date.now(),
    mutual: true
  }, { merge: true });
